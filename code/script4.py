...
def process(args):
    ...
    if 'temporal' in cfg:
        allowed_temporal = ['Peak to peak distance', 'Zero crossing rate']
        cfg['temporal'] = {k: v for k, v in cfg['temporal'].items() if k in allowed_temporal}
        
    if 'statistical' in cfg:
        allowed_stat = ['Mean', 'Standard deviation', 'Max', 'Min', 'Variance', 'Root mean square']
        cfg['statistical'] = {k: v for k, v in cfg['statistical'].items() if k in allowed_stat}
        
    if 'spectral' in cfg:
        allowed_spectral = ['Spectral centroid', 'Spectral roll-off']
        cfg['spectral'] = {k: v for k, v in cfg['spectral'].items() if k in allowed_spectral}
    ...
    timestamp_summary = sensor_df.select(["timestamp","time"]).with_row_index("idx").with_columns(
        (pl.col("idx") // points_per_sec).cast(pl.Int64).alias("id")
    ).group_by("id").agg(
...
def create_csv_file(output_filename, target="cutting_depth"):
    ...
    for arg in tasks:
        dfs_extracted.append(process(arg))
    
    result = pl.concat(dfs_extracted).collect()
    result = result.drop_nulls(subset=["y"])
    
    meta_cols = ["id", "sensor_file", "timestamp", "time", "y"]
        
    x_pd = result.drop(meta_cols).to_pandas()
...
def plot_csv(filename):
    ...
    axes[0].scatter(
        data["x"],
        data["Sound_mean"] if "Sound_mean" in data.columns else data.select(pl.selectors.contains("Sound")).columns[0],
        c=color_codes,
        cmap="tab10",
        s=5,
    ...
    axes[1].scatter(
        data["x"],
        data["Sound__mean"] if "Sound__mean" in data.columns else data.select(pl.selectors.contains("Sound")).columns[1],
        c=color_codes,
        cmap="tab10",
        s=5,
    ...
    axes[2].scatter(
        data["x"],
        data["Sound__maximum"] if "Sound__maximum" in data.columns else data.select(pl.selectors.contains("Sound")).columns[2],
        c=color_codes,
        cmap="tab10",
        s=5,
